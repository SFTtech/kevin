...
from kevin.util import SSHKnownHostFile, INF
...
class DockerContainerConfig(ContainerConfig):
    ...
    def __init__(self, machine_id, cfg, cfgpath):
        ...
        if not self.socket_uri:
            ...
            self.socket_uri = "unix://var/run/docker.sock"

        assert self.image_name is not None, \
            "Falk docker provider configuration error, " \
            "at least an image name is required"

        if self.context_path is not None:
            if self.dockerfile is None:
                self.dockerfile = "Dockerfile"
            self.context_path = Path(self.context_path)
            dockerfile = self.context_path / self.dockerfile
            if not dockerfile.is_file():
                raise ValueError(
                    "Dockerfile '{}' not found".format(str(dockerfile)))
